/**
 * eval/providers/openai.mjs — OpenAI Chat Completions provider driver
 *
 * ─── INLINE DUPLICATES ───────────────────────────────────────────────────────
 *
 ...
 *
 * buildPrompt() is a placeholder variant based on the Anthropic prompt in:
 *   extension/src/service-worker/rubric-prompt.ts
 *
 * SYNC REQUIRED: Re-sync this prompt against the finalized SD-033 implementation
 * in extension/src/service-worker/providers/ before the Wave 3 parity run.
 * This placeholder matches the base Anthropic prompt — SD-033 may add a
 * JSON schema reminder suffix or other OpenAI-specific adjustments.
 *
 * ─────────────────────────────────────────────────────────────────────────────
 */
...
/**
 * Pricing constants (as of 2026-04-20, gpt-4o-mini placeholder).
 * SD-033 will verify via live curl and may correct these.
 * Update when OpenAI changes pricing.
 */
export const input_price_per_million = 0.15
export const output_price_per_million = 0.60

export const RUBRIC_VERSION = 'rubric_v1.0-openai'
...
// ─── Prompt builder ───────────────────────────────────────────────────────────
//
// PLACEHOLDER: Mirrors the base Anthropic prompt.
// SD-033 may add a JSON schema reminder suffix or other OpenAI-specific
// adjustments. Re-sync before the Wave 3 parity run.
...
/**
 * Build the rubric prompt for a single text (sentence-level evaluation).
 *
 * @param {string} text
 * @returns {string}
 */
...
/**
 * Normalise OpenAI's usage field to the harness-internal shape.
 *
 * OpenAI returns: { prompt_tokens, completion_tokens, total_tokens }
 *
 * @param {object} apiResponse - Raw JSON response from OpenAI
...
/**
 * Call the OpenAI Chat Completions API and return parsed text + token usage.
 *
 * Uses response_format: { type: "json_object" } (JSON mode).
 * OpenAI models occasionally prepend prose before the JSON object even in JSON
 * mode — the caller (rubric-driver) handles parse failures by logging + skipping
