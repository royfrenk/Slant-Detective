/**
 * eval/providers/anthropic.mjs — Anthropic provider driver for the eval harness
 *
 * Extracted from eval/rubric-driver.mjs. The orchestration layer
 * (scoreBatch / scoreText) remains in rubric-driver.mjs until Task 2.
 *
 * ─── INLINE DUPLICATES ───────────────────────────────────────────────────────
 *
 ...
 *
 * Re-sync before Wave 3 parity run when SD-033 / SD-034 finalize their prompts.
 *
 * ─────────────────────────────────────────────────────────────────────────────
 */
...
/** Pricing constants (as of 2026-04-20). Update when Anthropic changes pricing. */
export const input_price_per_million = 0.80
export const output_price_per_million = 4.00
...
/**
 * Validate that the API key looks structurally valid (non-empty).
 * The harness does not perform a live test call.
 *
 * @param {string} apiKey
 */
...
/**
 * Normalise Anthropic's usage field to the harness-internal shape.
 *
 * Anthropic returns: { input_tokens, output_tokens }
 *
 * @param {object} apiResponse - Raw JSON response from Anthropic
...
/**
 * Call the Anthropic Messages API and return parsed text + token usage.
 *
...
export async function complete(text, apiKey, model = DEFAULT_MODEL) {
  ...
  return { text: rawText, usage: parseTokenUsage(json) }
}
